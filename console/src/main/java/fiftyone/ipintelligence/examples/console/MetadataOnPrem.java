...
public class MetadataOnPrem {
    ...
    public static void run(String dataFile, OutputStream output) throws Exception {
        ...
        try (IPIntelligenceOnPremiseEngine ipiEngine =
            ...
            outputComponents(ipiEngine, writer);
            writer.println();
            writer.flush();

            logger.info("Listing Profile Details");
            outputProfileDetails(ipiEngine, writer);
            writer.println();
            writer.flush();

            logger.info("Listing Evidence Key Details");
            outputEvidenceKeyDetails(ipiEngine, writer);
            writer.println();
            writer.flush();
    ...
    private static void outputEvidenceKeyDetails(IPIntelligenceOnPremiseEngine ipiEngine,
                                              PrintWriter output){
        output.println();
        if (ipiEngine.getEvidenceKeyFilter() instanceof EvidenceKeyFilterWhitelist) {
            // If the evidence key filter extends EvidenceKeyFilterWhitelist then we can
            // display a list of accepted keys.
            EvidenceKeyFilterWhitelist filter = (EvidenceKeyFilterWhitelist) ipiEngine.getEvidenceKeyFilter();
            ...
            output.format("The evidence key filter has type " +
                    ...
                    "or not.\n", ipiEngine.getEvidenceKeyFilter().getClass().getName());
            output.println("For example, query.client-ip " +
                    (ipiEngine.getEvidenceKeyFilter().include("query.client-ip") ?
                            "is " : "is not ") + "accepted.");
        }
    }

    private static void outputProfileDetails(IPIntelligenceOnPremiseEngine ipiEngine,
                                            PrintWriter output) {
        // Group the profiles by component and then output the number of profiles 
        // for each component.
        Map<String, List<ProfileMetaData>> groups =
                StreamSupport.stream(ipiEngine.getProfiles().spliterator(), false)
    ...
    // more readable.
    private static void outputComponents(IPIntelligenceOnPremiseEngine ipiEngine, PrintWriter output){
        ipiEngine.getComponents().forEach(c -> {
